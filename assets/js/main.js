$(function(){
    ...
    /*-------------------------------------------------------------------*/
    var serviceCollapse = function (){
        var ww = Math.max($(window).width(), window.innerWidth),
        serviceItem = $('.collapse:not(:first)', '#service');

        if (ww < 768){
            serviceItem.collapse('show');
        }
        else{
            serviceItem.collapse('hide');
        }
    };
    
    // Call serviceCollapse() when window is loaded.
    $(window).smartload(function(){
        serviceCollapse();
    });
    
    // Call serviceCollapse() when window is resized.
    $(window).smartresize(function(){
        serviceCollapse();
    });
